...
/**
 * Проверка на возможность пользователя удальять / создавать запись об отчете
 */
@Injectable()
export class DataScientistShopGuard implements CanActivate {
	...
	canActivate(
		...
		try {
			...
			const shopId = Number(url.split('/').at(-2));
